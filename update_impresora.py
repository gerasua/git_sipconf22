...
class updateDatosImpresora(QDialog):
    def __init__(self, parent=None):
        super(updateDatosImpresora, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('update_impresora.ui', self)
        logger.debug("Ventana actualizar Datos de la impresora")
        self.DB_Connect()
        ...
        try:
            ...
            self.paridad = self.row[5]
            logger.debug(f"Paridad: {self.paridad}")
            ##################################################################################

            self.bitsparada = self.row[4]
                ...
                try:
                    ...
                    if self.puertobd == self.detectport:
                        self.puertoBdDetectConfigurado = str(
                            self.port) + " (Detectado y configurado en la BD para la impresora)"
        ...
        ###
        try:
            logger.debug(f"Inicia Bits de datos = {self.bitsdatos}")
            if int(self.bitsdatos) == 4:
                logger.debug(f"Bits de datos cadena: {self.bitsdatos}")
                self.bitsdatosInBd = str(self.bitsdatos) + " (Configurado en BD)"
                self.comboBox_bitsDatos.addItem(str(self.bitsdatosInBd), str(self.bitsdatos))
            elif int(self.bitsdatos) == 5:
                logger.debug(f"Bits de datos: {self.bitsdatos}")
                self.bitsdatosInBd = str(self.bitsdatos) + " (Configurado en BD)"
                self.comboBox_bitsDatos.addItem(str(self.bitsdatosInBd), str(self.bitsdatos))
            elif int(self.bitsdatos) == 6:
                logger.debug(f"Bits de datos: {self.bitsdatos}")
                self.bitsdatosInBd = str(self.bitsdatos) + " (Configurado en BD)"
                self.comboBox_bitsDatos.addItem(str(self.bitsdatosInBd), str(self.bitsdatos))
            elif int(self.bitsdatos) == 7:
                logger.debug(f"Bits de datos: {self.bitsdatos}")
                self.bitsdatosInBd = str(self.bitsdatos) + " (Configurado en BD)"
                self.comboBox_bitsDatos.addItem(str(self.bitsdatosInBd), str(self.bitsdatos))
            elif int(self.bitsdatos) == 8:
                logger.debug(f"Bits de datos: {self.bitsdatos}")
                self.bitsdatosInBd = str(self.bitsdatos) + " (Configurado en BD)"
                self.comboBox_bitsDatos.addItem(str(self.bitsdatosInBd), str(self.bitsdatos))
            else:
        ...
        try:
            ...
            for self.resultado in self.resultados:
                if int(self.resultado) != int(self.bitsdatos):
    ...
    def agregar(self):
        ...
        try:
            query = """ UPDATE totalco_puertocom SET puerto = %s, baudrate = %s, bytesize = %s, parity = %s, timeout = %s WHERE id_puerto = %s """
            data = (str(self.capturaElPuerto), str(self.capturbitsxsegundo), str(self.capturabitsdedatos1), str(self.capturaparidadForBD), str(self.capturabitsdeparada), self.id)
