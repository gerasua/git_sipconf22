...
class updateSucursal(QDialog):
    def __init__(self, parent=None):
        super(updateSucursal, self).__init__(parent, flags=Qt.WindowCloseButtonHint)
        loadUi('update_sucursal.ui', self)
        ...
        try:
            ...
            logger.debug("Sucursal: {}".format(self.row_authenticate[3]))
            self.sucursal = self.row_authenticate[3]
            self.lineUpdateSucursal.setText(str(self.sucursal))
    ...
    def actualizar(self):
        ...
        self.itemUpdate = self.lineUpdateSucursal.displayText()
        logger.debug("Nombre: {}".format(self.itemUpdate))

        if self.itemUpdate:
            logger.debug("No esta vacío")
            try:
                query = """ UPDATE totalco_info_gral SET sucursal = %s WHERE id_info_gral = %s """
            ...
            self.lineUpdateSucursal.setFocus()
